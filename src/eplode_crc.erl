...
% CRC algorithm is due to Chuck Forsberg, Omen Technology.
% Table calculated by Mark G. Mendel, Network Systems Corporation.
% The update step is derived from an article Copyright (c) 1986 Stephen Satchell.
...
crc16(Value) ->
    crc16(iolist_to_binary(Value), 0).

crc16(<<>>, CRC) ->
    CRC;
crc16(<<Value:8, Rest/binary>>, CRC) when Value =< 16#FF ->
    Index = (CRC bsr 8) band 255,
    Next = element(Index + 1, ?CRC16_TAB) bxor (CRC bsl 8) bxor Value,
    crc16(Rest, Next band 16#FFFF).
